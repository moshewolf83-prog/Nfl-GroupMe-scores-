...
BOT_VERSION = "2026-08-14-retry-fix-v2"
...
def get_win_pct_from_kalshi(team_full_name, opponent_full_name=None):
    ...
    # Prefer a market that also mentions the opponent (more likely the actual
    # game moneyline rather than an unrelated season-long market)
    if opponent_full_name:
        opp_words = [w.lower() for w in opponent_full_name.split() if len(w) > 2]
        narrowed = [m for m in candidates if any(w in m.get("title", "").lower() for w in opp_words)]
        if narrowed:
            candidates = narrowed
    ...
    for event in events:
        ...
        for name, prob in probs.items():
            if team_full_name.lower() in name.lower():
                return round((prob / total) * 100)  # normalized, vig removed
    return None
...
def get_win_pct_for_event(sport, league, event_id, abbr):
    ...
    if home_abbr == abbr and home_pct is not None:
        return round(float(home_pct))
    if away_abbr == abbr and away_pct is not None:
        return round(float(away_pct))
    return None
