...
BOT_VERSION = "2026-08-14-winpct-sanity-v3"
...
TEAM_ALIASES = {
    ...
    "cavaliers": ("basketball", "nba", "CLE"), "cavs": ("basketball", "nba", "CLE"),
    "mavericks": ("basketball", "nba", "DAL"), "mavs": ("basketball", "nba", "DAL"),
    ...
    "timberwolves": ("basketball", "nba", "MIN"), "wolves": ("basketball", "nba", "MIN"),
    "pelicans": ("basketball", "nba", "NO"),
    "knicks": ("basketball", "nba", "NY"),
    "thunder": ("basketball", "nba", "OKC"),
    "magic": ("basketball", "nba", "ORL"),
    "76ers": ("basketball", "nba", "PHI"), "sixers": ("basketball", "nba", "PHI"),
    "suns": ("basketball", "nba", "PHX"),
    "trail blazers": ("basketball", "nba", "POR"), "blazers": ("basketball", "nba", "POR"),
    "sacramento kings": ("basketball", "nba", "SAC"),
    "spurs": ("basketball", "nba", "SA"),
    "raptors": ("basketball", "nba", "TOR"),
    "jazz": ("basketball", "nba", "UTAH"),
    "wizards": ("basketball", "nba", "WSH"),
    # NHL
    "ducks": ("hockey", "nhl", "ANA"),
    "bruins": ("hockey", "nhl", "BOS"),
    "sabres": ("hockey", "nhl", "BUF"),
    "flames": ("hockey", "nhl", "CGY"),
    "hurricanes": ("hockey", "nhl", "CAR"), "canes": ("hockey", "nhl", "CAR"),
    ...
    "la kings": ("hockey", "nhl", "LA"),
    "wild": ("hockey", "nhl", "MIN"),
    "canadiens": ("hockey", "nhl", "MTL"), "habs": ("hockey", "nhl", "MTL"),
    "predators": ("hockey", "nhl", "NSH"), "preds": ("hockey", "nhl", "NSH"),
    "devils": ("hockey", "nhl", "NJ"),
    "islanders": ("hockey", "nhl", "NYI"),
    "ny rangers": ("hockey", "nhl", "NYR"),
    "senators": ("hockey", "nhl", "OTT"), "sens": ("hockey", "nhl", "OTT"),
    "flyers": ("hockey", "nhl", "PHI"),
    "penguins": ("hockey", "nhl", "PIT"), "pens": ("hockey", "nhl", "PIT"),
    "sharks": ("hockey", "nhl", "SJ"),
    "kraken": ("hockey", "nhl", "SEA"),
    "blues": ("hockey", "nhl", "STL"),
    "lightning": ("hockey", "nhl", "TB"),
    "maple leafs": ("hockey", "nhl", "TOR"), "leafs": ("hockey", "nhl", "TOR"),
    "canucks": ("hockey", "nhl", "VAN"),
    "golden knights": ("hockey", "nhl", "VGK"), "knights": ("hockey", "nhl", "VGK"),
    "capitals": ("hockey", "nhl", "WSH"), "caps": ("hockey", "nhl", "WSH"),
...
def get_team_schedule(sport, league, abbr, count=10):
    try:
    ...
    for event in events:
        ...
        competitions = event.get("competitions", [{}])[0]
        competitors = competitions.get("competitors", [])
        opp = next((c for c in competitors if c.get("team", {}).get("abbreviation") != abbr), None)
        me = next((c for c in competitors if c.get("team", {}).get("abbreviation") == abbr), None)
        opp_abbr = opp.get("team", {}).get("abbreviation", "?") if opp else "?"
        at_vs = "@" if me and me.get("homeAway") == "away" else "vs"
        ...
        upcoming.append((game_date, f"{at_vs} {opp_abbr} - {when}"))

    if not upcoming:
        return f"No upcoming games found for {abbr}."

    upcoming.sort(key=lambda x: x[0])
    lines = [line for _, line in upcoming[:count]]
    return f"{abbr} schedule:\n" + "\n".join(lines)


def get_stat_leaders(sport, league, stat_query, count=10):
    url = f"https://site.api.espn.com/apis/site/v2/sports/{sport}/{league}/leaders"
    try:
        resp = requests.get(url, params={"season": ESPN_SEASON}, timeout=10)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        return f"Couldn't fetch leaders right now ({e})"

    categories = data.get("leaders", [])
    if not categories:
        return "Couldn't find leader data right now."

    stat_query_lower = stat_query.lower().strip()
    matched = None
    for cat in categories:
        names = [
            cat.get("name", ""), cat.get("displayName", ""),
            cat.get("shortDisplayName", ""), cat.get("abbreviation", ""),
        ]
        if any(stat_query_lower in n.lower() for n in names if n):
            matched = cat
            break

    if not matched:
        available = ", ".join(c.get("shortDisplayName", c.get("name", "?")) for c in categories[:10])
        return f"Couldn't match \"{stat_query}\". Try one of: {available}"

    leaders = matched.get("leaders", [])[:count]
    if not leaders:
        return f"No leader data available for {matched.get('displayName', stat_query)}."
    ...
    for i, leader in enumerate(leaders, 1):
        ...
        lines.append(f"{i}. {athlete} ({team}) - {value}")

    label = matched.get("displayName", stat_query)
    return f"{label} leaders:\n" + "\n".join(lines)
...
def get_team_roster(sport, league, abbr):
    url = f"https://site.api.espn.com/apis/site/v2/sports/{sport}/{league}/teams/{abbr}/roster"
    try:
        resp = requests.get(url, timeout=10)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        return f"Couldn't fetch roster right now ({e})"
    ...
    def add_player(p):
        ...
        if name:
            players.append(f"{name} ({pos})" if pos else name)
    ...
    if not players:
        return f"Couldn't find a roster for {abbr}."

    MAX_PLAYERS = 20
    shown = players[:MAX_PLAYERS]
    text = f"{abbr} roster:\n" + "\n".join(shown)
    if len(players) > MAX_PLAYERS:
        text += f"\n...and {len(players) - MAX_PLAYERS} more"
    return text
...
def search_player_stats_anywhere(name_fragment):
    """No pre-mapping needed - scans every currently live NFL and MLB game's
    box score for a matching player name."""
    for sport, league in (("baseball", "mlb"), ("football", "nfl")):
        try:
            resp = requests.get(scoreboard_url(sport, league), timeout=10)
            resp.raise_for_status()
            events = resp.json().get("events", [])
        ...
        for event in events:
            ...
            if not event_id:
                continue
            try:
                summary_resp = requests.get(summary_url(sport, league, event_id), timeout=10)
                ...
                for stat_group in team_block.get("statistics", []):
                    ...
                    for athlete_entry in stat_group.get("athletes", []):
                        ...
                        if not stats and not summary_text:
                            continue
                        line = (
                            format_batting_line(labels, stats, summary_text)
                            if league == "mlb"
                            else format_football_stat_line(labels, stats, summary_text)
                        )
                        return f"{name}: {line}"

    return f"Couldn't find \"{name_fragment}\" in any live game's box score right now (they may not be playing, or the game hasn't started)."
...
def get_team_result(sport, league, abbr):
    ...
    for event in data.get("events", []):
        ...
        if abbr in abbrs:
            competitions = event.get("competitions", [{}])[0]
            state = competitions.get("status", {}).get("type", {}).get("state")
            is_live = state == "in"
            ...
            if is_live:
                event_id = event.get("id")
                win_pct = get_win_pct_for_event(sport, league, event_id, abbr)
                if win_pct is not None:
                    line += f"\n{abbr} {win_pct}% chance of winning"
...
def get_nba_standings():
    return get_standings("basketball", "nba")


def get_nhl_standings():
    return get_standings("hockey", "nhl")
...
def debug_playoff_odds(sport, league, abbr):
    metric_name = "fpi" if league == "nfl" else "bpi"
    url = f"https://site.api.espn.com/apis/fitt/v3/sports/{sport}/{league}/{metric_name}"
    try:
        resp = requests.get(url, timeout=10)
    ...
    try:
        ...
        return f"Not valid JSON. Raw start: {resp.text[:200]}"
    ...
    if not sample:
        return f"Top-level keys: {top_keys}. {len(teams)} teams found, but none matched {abbr}. First team keys (if any): {list(teams[0].keys()) if teams else 'none'}"

    return f"Found {abbr}. Entry keys: {list(sample.keys())}. Stats keys (if present): {list(sample.get('stats', {}).keys()) if isinstance(sample.get('stats'), dict) else sample.get('stats')}"
...
def get_win_pct_from_kalshi(team_full_name, opponent_full_name=None):
    url = "https://api.elections.kalshi.com/trade-api/v2/markets"
    try:
        resp = requests.get(url, params={"limit": 200, "status": "open"}, timeout=15)
        resp.raise_for_status()
        markets = resp.json().get("markets", [])
    except Exception:
        return None

    team_words = [w.lower() for w in team_full_name.split() if len(w) > 2]

    candidates = []
    for m in markets:
        title = m.get("title", "").lower()
        if any(w in title for w in team_words):
            candidates.append(m)

    if not candidates:
        return None

    # Require the opponent's name too - this is the difference between a real
    # game moneyline and an unrelated season-long/award market that just
    # happens to mention the team. Without this, we've matched garbage before.
    if opponent_full_name:
        opp_words = [w.lower() for w in opponent_full_name.split() if len(w) > 2]
        candidates = [m for m in candidates if any(w in m.get("title", "").lower() for w in opp_words)]
    else:
        return None  # too risky to guess without knowing the opponent

    if not candidates:
        return None

    pct = kalshi_price_pct(candidates[0])
    if pct is None or pct <= 0 or pct >= 100:
        return None  # sanity check - a real live moneyline is never exactly 0 or 100
    return pct


def get_win_pct_from_odds_api(league, team_full_name):
    if not ODDS_API_KEY:
        return None
    ...
    for event in events:
        teams_in_event = {event.get("home_team", ""), event.get("away_team", "")}
        if not any(team_full_name.lower() in t.lower() for t in teams_in_event):
            continue
        ...
        for outcome in outcomes:
            probs[outcome["name"]] = american_odds_to_implied_prob(outcome["price"])
        total = sum(probs.values())
        for name, prob in probs.items():
            if team_full_name.lower() in name.lower():
                pct = round((prob / total) * 100)  # normalized, vig removed
                return pct if 0 < pct < 100 else None
    return None


def get_scoring_plays(sport, league, abbr):
    event_id = find_team_event_id(sport, league, abbr)
    if not event_id:
        return f"{abbr} isn't playing right now."
    ...
    try:
        ...
        return f"Couldn't fetch scoring plays right now ({e})"

    scoring_plays = summary.get("scoringPlays", [])
    if not scoring_plays:
        return f"No scoring plays logged yet for {abbr}'s game."

    lines = []
    for play in scoring_plays[-15:]:  # most recent 15
        text = play.get("text", "").strip()
        away_score = play.get("awayScore")
        home_score = play.get("homeScore")
        if away_score is not None and home_score is not None:
            lines.append(f"{text} ({away_score}-{home_score})")
        else:
            lines.append(text)

    return f"{abbr} scoring plays:\n" + "\n".join(lines)


def get_win_pct_for_event(sport, league, event_id, abbr):
    """Returns an int percent for the given team, or None if unavailable.
    Tries Kalshi first (free, unlimited), then real sportsbook odds (if
    configured), then falls back to ESPN's predictor as a last resort."""
    try:
        resp = requests.get(summary_url(sport, league, event_id), timeout=10)
        resp.raise_for_status()
        summary = resp.json()
    ...
    if competitions:
        for competitor in competitions[0].get("competitors", []):
            if competitor.get("team", {}).get("abbreviation") == abbr:
                team_full_name = competitor.get("team", {}).get("displayName")
            else:
                opponent_full_name = competitor.get("team", {}).get("displayName")

    if team_full_name:
        kalshi_pct = get_win_pct_from_kalshi(team_full_name, opponent_full_name)
        if kalshi_pct is not None:
            return kalshi_pct

        odds_pct = get_win_pct_from_odds_api(league, team_full_name)
        if odds_pct is not None:
            return odds_pct
    ...
    home_abbr = predictor.get("homeTeam", {}).get("team", {}).get("abbreviation")
    away_abbr = predictor.get("awayTeam", {}).get("team", {}).get("abbreviation")

    if home_abbr == abbr and home_pct is not None:
        pct = round(float(home_pct))
        return pct if 0 < pct < 100 else None
    if away_abbr == abbr and away_pct is not None:
        pct = round(float(away_pct))
        return pct if 0 < pct < 100 else None
    return None
...
def get_one_fantasy_matchup(league_id, team_id, label):
    ...
    try:
        ...
        retries = 0
        while resp.status_code == 202 and retries < 4:
            time.sleep(2 + retries)  # 2s, 3s, 4s, 5s - backs off progressively
            resp = requests.get(url, cookies=cookies, headers=headers, timeout=15)
            retries += 1
    except Exception as e:
...
def find_free_agent_id(name_fragment):
    """Search ESPN's global player pool (reuses same approach as fantasy points lookup)."""
    season = ESPN_SEASON
    ...
    for player in players:
        ...
        if name_fragment.lower() in full_name.lower():
            return player.get("id"), full_name
    return None, None


POSITION_SLOT_IDS = {"qb": 0, "rb": 2, "wr": 4, "te": 6, "dst": 16, "k": 17}


def get_top_free_agents(league_id, position_abbr, count=10):
    slot_id = POSITION_SLOT_IDS.get(position_abbr.lower())
    if slot_id is None:
        return "Use one of: qb, rb, wr, te, dst, k."

    cookies, headers = espn_fantasy_auth()
    headers["x-fantasy-filter"] = (
        '{"players":{"filterSlotIds":{"value":[' + str(slot_id) + ']},'
        '"filterStatus":{"value":["FREEAGENT","WAIVERS"]},'
        '"sortPercOwned":{"sortPriority":1,"sortAsc":false},'
        f'"limit":{count}}}'
    )
    url = f"https://fantasy.espn.com/apis/v3/games/ffl/seasons/{ESPN_SEASON}/segments/0/leagues/{league_id}?view=kona_player_info"

    try:
        resp = requests.get(url, cookies=cookies, headers=headers, timeout=15)
        if resp.status_code != 200:
            return f"Couldn't fetch free agents (status {resp.status_code}): {resp.text[:150]}"
        data = resp.json()
    except Exception as e:
        return f"Couldn't fetch free agents ({e})"

    players = data.get("players", [])
    if not players:
        return f"No available {position_abbr.upper()}s found (or the season/league data isn't ready yet)."

    lines = []
    for entry in players[:count]:
        player = entry.get("player", entry)
        name = player.get("fullName", "?")
        pct_owned = player.get("ownership", {}).get("percentOwned")
        pct_text = f" ({round(pct_owned)}% owned)" if pct_owned is not None else ""
        lines.append(f"{name}{pct_text}")

    return f"Top available {position_abbr.upper()}s:\n" + "\n".join(lines)


def add_free_agent(league_id, team_id, add_name, drop_name=None):
    if not ESPN_S2 or not ESPN_SWID:
        return "Adding players needs ESPN_S2 and ESPN_SWID set up first."

    add_id, add_full_name = find_free_agent_id(add_name)
    if not add_id:
        return f"Couldn't find a player matching \"{add_name}\"."

    drop_id, drop_full_name = None, None
    if drop_name:
        entries, err = get_fantasy_roster_detail(league_id, team_id)
        if err:
            return err
        drop_entry = next((e for e in entries if drop_name.lower() in e["name"].lower()), None)
        if not drop_entry:
            return f"Couldn't find \"{drop_name}\" on your roster to drop."
        drop_id = drop_entry["player"].get("id")
        drop_full_name = drop_entry["name"]
    ...
    try:
        status_resp = requests.get(
            f"https://fantasy.espn.com/apis/v3/games/ffl/seasons/{ESPN_SEASON}/segments/0/leagues/{league_id}",
            cookies=cookies, headers=headers, timeout=10,
        )
        current_period = status_resp.json().get("status", {}).get("currentMatchupPeriod")
    except Exception:
        pass

    items = [{"playerId": add_id, "type": "ADD", "toLineupSlotId": 20}]
    if drop_id:
        items.append({"playerId": drop_id, "type": "DROP", "fromLineupSlotId": 20})

    body = {
        "isLeagueManager": False,
        "teamId": int(team_id),
        "type": "WAIVER" if False else "FREEAGENT",
        "memberId": ESPN_SWID,
        "items": items,
    ...
    try:
        resp = requests.post(
            f"https://fantasy.espn.com/apis/v3/games/ffl/seasons/{ESPN_SEASON}/segments/0/leagues/{league_id}/transactions/",
            cookies=cookies, headers=headers, json=body, timeout=15,
        )
        if resp.status_code not in (200, 201):
            return f"Add failed (status {resp.status_code}): {resp.text[:200]}"
    except Exception as e:
        return f"Add failed ({e})"

    if drop_full_name:
        return f"Added {add_full_name}, dropped {drop_full_name}."
    return f"Added {add_full_name}."


def switch_lineup_players(league_id, team_id, name1, name2):
    if not ESPN_S2 or not ESPN_SWID:
        return "Switching lineups needs ESPN_S2 and ESPN_SWID set up first (required even for public leagues)."

    entries, err = get_fantasy_roster_detail(league_id, team_id)
    if err:
        return err

    def find_entry(name_fragment):
        return next((e for e in entries if name_fragment.lower() in e["name"].lower()), None)

    e1 = find_entry(name1)
    e2 = find_entry(name2)
    if not e1:
        return f"Couldn't find \"{name1}\" on your roster."
    if not e2:
        return f"Couldn't find \"{name2}\" on your roster."

    slot_id_map = {v: k for k, v in LINEUP_SLOT_NAMES.items()}
    from_slot1 = slot_id_map.get(e1["slot"])
    from_slot2 = slot_id_map.get(e2["slot"])
    if from_slot1 is None or from_slot2 is None:
        return "Couldn't determine lineup slots for one of those players."

    if from_slot1 == from_slot2:
        return f"{e1['name']} and {e2['name']} are already in the same slot."

    player1_id = e1["player"].get("id")
    player2_id = e2["player"].get("id")
    cookies, headers = espn_fantasy_auth()
    headers["Content-Type"] = "application/json"
    ...
    body = {
        ...
        "memberId": ESPN_SWID,
        "items": [
            {"playerId": player1_id, "type": "LINEUP", "fromLineupSlotId": from_slot1, "toLineupSlotId": from_slot2},
            {"playerId": player2_id, "type": "LINEUP", "fromLineupSlotId": from_slot2, "toLineupSlotId": from_slot1},
        ],
        "scoringPeriodId": current_period,
        "executionType": "EXECUTE",
    }
    ...
    try:
        ...
        if resp.status_code not in (200, 201):
            return f"Switch failed (status {resp.status_code}): {resp.text[:200]}"
    except Exception as e:
        return f"Switch failed ({e})"

    return f"Switched {e1['name']} <-> {e2['name']}."
...
LINEUP_SLOT_NAMES = {
    ...
    20: "BN", 21: "IR", 23: "FLEX", 25: "SUPERFLEX",
}


def espn_fantasy_auth():
    cookies = {}
    if ESPN_S2 and ESPN_SWID:
        cookies = {"espn_s2": ESPN_S2, "SWID": ESPN_SWID}
    headers = {
...
def get_team_bye_week(abbr):
    try:
        resp = requests.get(schedule_url("football", "nfl", abbr), timeout=10)
        resp.raise_for_status()
        events = resp.json().get("events", [])
    except Exception:
        return None
...
def get_fantasy_roster_detail(league_id, team_id, week=None, mode="actual"):
    cookies, headers = espn_fantasy_auth()
    week_param = f"&scoringPeriodId={week}" if week else ""
    url = (
        f"https://fantasy.espn.com/apis/v3/games/ffl/seasons/{ESPN_SEASON}"
        f"/segments/0/leagues/{league_id}?view=mRoster&view=mTeam{week_param}"
    )
    try:
        resp = requests.get(url, cookies=cookies, headers=headers, timeout=15)
        retries = 0
        ...
        return None, f"Couldn't reach ESPN right now ({e})"

    if resp.status_code == 202:
        return None, "ESPN hasn't finished preparing this data yet (common before the season/draft is fully active) - try again later, this isn't a cookie problem."

    if resp.status_code in (401, 403):
        return None, f"ESPN rejected the request (status {resp.status_code}) - this usually does mean a bad or expired ESPN_S2/ESPN_SWID cookie."

    if resp.status_code != 200:
        return None, f"ESPN returned status {resp.status_code}. Response: {resp.text[:150]}"

    try:
        data = resp.json()
    except Exception:
        return None, f"Got a non-JSON response. Response start: {resp.text[:150]}"

    team_id_int = int(team_id)
    team = next((t for t in data.get("teams", []) if t.get("id") == team_id_int), None)
    if not team:
        return None, "Couldn't find your team in that league."

    current_period = week or data.get("status", {}).get("currentMatchupPeriod")
    source_id = 1 if mode == "projected" else 0
    ...
    for entry in team.get("roster", {}).get("entries", []):
        ...
        slot_name = LINEUP_SLOT_NAMES.get(slot_id, str(slot_id))

        points = None
        for stat in player.get("stats", []):
            if stat.get("scoringPeriodId") == current_period and stat.get("statSourceId") == source_id:
                points = stat.get("appliedTotal")
                break

        entries.append({"name": name, "slot": slot_name, "points": points, "player": player})

    return entries, None


def format_roster_text(entries, week_label):
    starters = [e for e in entries if e["slot"] not in ("BN", "IR")]
    bench = [e for e in entries if e["slot"] == "BN"]

    def line(e):
        pts = f"{round(e['points'], 1)}" if e["points"] is not None else "-"
        return f"{e['slot']}: {e['name']} - {pts} pts"

    lines = [f"Starters ({week_label}):"]
    lines += [line(e) for e in starters] or ["(none)"]
    lines.append("")
    lines.append("Bench:")
    lines += [line(e) for e in bench] or ["(none)"]
    return "\n".join(lines)


def get_bye_week_players(league_id, team_id, week):
    entries, err = get_fantasy_roster_detail(league_id, team_id)
    if err:
        return err
    ...
    checked_teams = {}
    for e in entries:
        pro_team_id = e["player"].get("proTeamId")
        abbr = ESPN_PRO_TEAM_ABBR.get(pro_team_id)
        if not abbr:
            continue
        if abbr not in checked_teams:
            checked_teams[abbr] = get_team_bye_week(abbr)
        if checked_teams[abbr] == week:
            matches.append(e["name"])

    if not matches:
        return f"No one on your roster has a bye in week {week}."
    return f"Bye in week {week}:\n" + "\n".join(matches)
...
# ---- Injury reports ----
STATUS_KEYWORDS = [
    "out", "questionable", "doubtful", "probable", "day-to-day",
    "injured reserve", "ir", "active", "game-time decision",
]


def extract_status_keyword(text):
    if not text:
        return None
...
def check_injury_conflict(player_name):
    sources = {}

    # Source 1: ESPN's structured injury report
    espn_status = None
    try:
        resp = requests.get("https://site.api.espn.com/apis/site/v2/sports/football/nfl/injuries", timeout=10)
        resp.raise_for_status()
        data = resp.json()
        for team_block in data.get("injuries", []):
            for inj in team_block.get("injuries", []):
                athlete_name = inj.get("athlete", {}).get("displayName", "")
                if player_name.lower() in athlete_name.lower():
                    espn_status = inj.get("status", "")
                    break
            if espn_status:
                break
    except Exception:
        pass
    sources["ESPN"] = espn_status or "no data"

    # Source 2: live web search
    web_result = do_search(f"{player_name} injury status")
    web_status = extract_status_keyword(web_result)
    sources["Web search"] = web_status or (web_result[:60] if web_result else "no data")

    # Source 3: news headlines
    news_result = get_news(f"{player_name} injury")
    news_status = extract_status_keyword(news_result)
    sources["News headlines"] = news_status or "no clear status"

    found_statuses = {
        v for v in (espn_status, web_status, news_status)
        if v and v.lower() not in ("no data", "no clear status")
    }

    lines = [f"{src}: {val}" for src, val in sources.items()]

    if len(found_statuses) <= 1:
        verdict = "Sources agree." if found_statuses else "No clear status found from any source."
    else:
        verdict = "Sources DISAGREE - verify before relying on this: " + ", ".join(found_statuses)

    return f"{player_name} injury status check:\n" + "\n".join(lines) + f"\n\n{verdict}"
...
# ---- Game day countdown ----
def get_h2h(sport, league, abbr1, abbr2):
    try:
        resp = requests.get(schedule_url(sport, league, abbr1), timeout=10)
        resp.raise_for_status()
        events = resp.json().get("events", [])
    except Exception as e:
        return f"Couldn't fetch head-to-head right now ({e})"
    ...
    for event in events:
        competitions = event.get("competitions", [{}])[0]
        competitors = competitions.get("competitors", [])
        abbrs_in_game = {c.get("team", {}).get("abbreviation") for c in competitors}
        if abbr2 not in abbrs_in_game:
            continue
        status = competitions.get("status", {}).get("type", {})
        if not status.get("completed"):
            continue
        team1 = next((c for c in competitors if c.get("team", {}).get("abbreviation") == abbr1), None)
        team2 = next((c for c in competitors if c.get("team", {}).get("abbreviation") == abbr2), None)
        if not team1 or not team2:
            continue
        s1, s2 = team1.get("score", "0"), team2.get("score", "0")
        won = team1.get("winner", False)
        if won:
            wins1 += 1
        games.append(f"{abbr1} {s1} - {abbr2} {s2}" + (" (W)" if won else " (L)"))

    if not games:
        return f"No completed games between {abbr1} and {abbr2} found this season."

    wins2 = len(games) - wins1
    header = f"{abbr1} vs {abbr2} this season: {wins1}-{wins2}"
    return header + "\n" + "\n".join(games[-5:])


def get_streak(sport, league, abbr):
    all_entries, err = get_flat_standings_entries(sport, league)
    if err:
        return err
    entry = next((e for e in all_entries if e.get("team", {}).get("abbreviation") == abbr), None)
    if not entry:
        return f"Couldn't find {abbr} in standings."
    streak = _stat_value(entry, {"streak"}, use_display=True)
    if not streak:
        return f"No streak data available for {abbr}."
    return f"{abbr} current streak: {streak}"


def get_home_away_split(sport, league, abbr):
    all_entries, err = get_flat_standings_entries(sport, league)
    if err:
        return err
    entry = next((e for e in all_entries if e.get("team", {}).get("abbreviation") == abbr), None)
    if not entry:
        return f"Couldn't find {abbr} in standings."
    home = _stat_value(entry, {"home"}, use_display=True)
    away = _stat_value(entry, {"road", "away"}, use_display=True)
    if not home and not away:
        return f"No home/away split available for {abbr}."
    return f"{abbr} - Home: {home or '?'}, Away: {away or '?'}"


TRADE_DEADLINES = {
    "mlb": datetime(2026, 8, 3, 18, 0, tzinfo=ZoneInfo("America/New_York")),
    "nfl": datetime(2026, 11, 10, 16, 0, tzinfo=ZoneInfo("America/New_York")),
}


def get_trade_deadline_countdown(league):
    deadline = TRADE_DEADLINES.get(league)
    if not deadline:
        return "No trade deadline data for that league."
    now = datetime.now(ZoneInfo("America/New_York"))
    if now >= deadline:
        return f"The {league.upper()} trade deadline already passed ({deadline.strftime('%b %d, %Y')})."
    delta = deadline - now
    return f"{league.upper()} trade deadline: {delta.days}d {delta.seconds // 3600}h remaining ({deadline.strftime('%b %d, %Y %-I:%M %p ET')})"


SEASON_TOTAL_GAMES = {"mlb": 162, "nfl": 17, "nba": 82, "nhl": 82}


def get_magic_number(sport, league, abbr):
    total_games = SEASON_TOTAL_GAMES.get(league)
    if not total_games:
        return "Magic number not supported for that league."
    ...
    division_abbrs = get_division_map(league)[div_name]
    records = []
    for a in division_abbrs:
        entry = next((e for e in all_entries if e.get("team", {}).get("abbreviation") == a), None)
        if not entry:
            continue
        wins = _stat_value(entry, {"wins"})
        losses = _stat_value(entry, {"losses"})
        wins = int(wins) if wins is not None else 0
        losses = int(losses) if losses is not None else 0
        records.append((a, wins, losses))

    if not records:
        return "Couldn't find division standings."

    records.sort(key=lambda r: r[1] / max(r[1] + r[2], 1), reverse=True)
    leader_abbr, leader_wins, leader_losses = records[0]

    if leader_abbr == abbr:
        if len(records) < 2:
            return f"{abbr} is alone in their division - no competitor to clinch against."
        _, _, chaser_losses = records[1]
        magic = (total_games + 1) - leader_wins - chaser_losses
        if magic <= 0:
            return f"{abbr} has already clinched the division!"
        return f"{abbr} magic number to clinch division: {magic}"
    else:
        my_entry = next((r for r in records if r[0] == abbr), None)
        if not my_entry:
            return f"Couldn't find {abbr}'s record."
        _, my_wins, my_losses = my_entry
        elim = (total_games + 1) - leader_wins - my_losses
        if elim <= 0:
            return f"{abbr} has been eliminated from winning the division."
        return f"{abbr} elimination number (vs {leader_abbr}): {elim} (need this many combined {abbr} wins + {leader_abbr} losses to stay alive)"


def get_player_season_points(name_fragment):
    """Rough season-total PPR points using the same global player search as fantasy points."""
    season = ESPN_SEASON
    url = f"https://fantasy.espn.com/apis/v3/games/ffl/seasons/{season}/players?view=kona_player_info"
    headers = {
        "User-Agent": (
        ...
        if name_fragment.lower() in full_name.lower():
            ...
            for stat_entry in player.get("stats", []):
                if stat_entry.get("statSourceId") == 0:
                    ...
                    pts = calc_ppr_points(raw)
                    if pts:
                        total += pts
            return full_name, round(total, 1)
    return None, None


def analyze_trade(side_a_names, side_b_names):
    def total_for(names):
        results = []
        total = 0.0
        for name in names:
            full_name, pts = get_player_season_points(name.strip())
            if full_name is None:
                results.append(f"{name.strip()}: not found")
            else:
                results.append(f"{full_name}: {pts} pts")
                total += pts
        return results, round(total, 1)

    a_lines, a_total = total_for(side_a_names)
    b_lines, b_total = total_for(side_b_names)

    verdict = "Even trade."
    if a_total > b_total:
        verdict = f"Side A gets the better value (+{round(a_total - b_total, 1)} pts)."
    elif b_total > a_total:
        verdict = f"Side B gets the better value (+{round(b_total - a_total, 1)} pts)."

    return (
        f"Side A ({a_total} pts):\n" + "\n".join(a_lines) +
        f"\n\nSide B ({b_total} pts):\n" + "\n".join(b_lines) +
        f"\n\n{verdict}"
    )


def get_clutch_record(sport, league, abbr):
    try:
        resp = requests.get(schedule_url(sport, league, abbr), timeout=10)
        resp.raise_for_status()
        events = resp.json().get("events", [])
    ...
    margin_threshold = 1 if league == "mlb" else 8  # 1-run games / one-score games
    wins, losses = 0, 0
    for event in events:
        competitions = event.get("competitions", [{}])[0]
        status = competitions.get("status", {}).get("type", {})
    ...
    total = wins + losses
    if total == 0:
        label = "one-run games" if league == "mlb" else "one-score games"
        return f"No {label} found for {abbr} this season."
    label = "one-run games" if league == "mlb" else "one-score games (\u22648 pts)"
    return f"{abbr} in {label}: {wins}-{losses}"
...
def check_alcohol_combo(ing1, ing2):
    """Uses TheCocktailDB's free public API (no signup needed)."""
    try:
        resp1 = requests.get(
            f"https://www.thecocktaildb.com/api/json/v1/1/filter.php?i={quote(ing1)}", timeout=10
        )
        resp2 = requests.get(
            f"https://www.thecocktaildb.com/api/json/v1/1/filter.php?i={quote(ing2)}", timeout=10
        )
        drinks1 = resp1.json().get("drinks") or []
        drinks2 = resp2.json().get("drinks") or []
    except Exception as e:
        return f"Couldn't check that right now ({e})"

    names1 = {d.get("strDrink") for d in drinks1 if d.get("strDrink")}
    names2 = {d.get("strDrink") for d in drinks2 if d.get("strDrink")}
    overlap = sorted(names1 & names2)

    if overlap:
        examples = ", ".join(overlap[:3])
        return f"Yes - {ing1} and {ing2} are a known combo. Try: {examples}"

    if names1 or names2:
        return (
            f"Couldn't find a well-known cocktail combining {ing1} and {ing2} specifically, "
            "but that doesn't necessarily mean it's bad."
        )
    return f"Couldn't find data for \"{ing1}\" or \"{ing2}\" - try more common ingredient names."


def parse_shots_query(text):
    match = re.search(r"(\d+(?:\.\d+)?)\s*shots?\s*(?:of\s*)?(\d+(?:\.\d+)?)\s*%?\s*(?:percent)?", text)
    if not match:
        return None
    return float(match.group(1)), float(match.group(2))


def alcohol_conversion(shot_count, abv, shot_oz=1.5):
    pure_alcohol_oz = shot_count * shot_oz * (abv / 100)
    beer_alcohol_oz = 12 * 0.05    # standard 12oz beer @ 5% ABV
    wine_alcohol_oz = 5 * 0.12     # standard 5oz wine @ 12% ABV
    beers = pure_alcohol_oz / beer_alcohol_oz
    wines = pure_alcohol_oz / wine_alcohol_oz
    return (
        f"{shot_count:g} shots of {abv:g}% \u2248 "
        f"{round(beers, 1)} standard beers (12oz, 5%)\n"
        f"or {round(wines, 1)} glasses of wine (5oz, 12%)\n"
        "(Drink responsibly - know your limits.)"
    )
...
def kalshi_price_pct(market):
    """Kalshi prices are in cents (0-100), representing implied probability."""
...
def kalshi_search(topic):
    ...
    try:
        ...
        return f"Couldn't reach Kalshi right now ({e})"
    ...
    found = [m for m in markets if matches(m)]
    if not found:
        return f"No open Kalshi market found for \"{topic}\"."

    # Group by event - if several markets share an event, show as a leaderboard
    from collections import defaultdict
    by_event = defaultdict(list)
    for m in found:
        by_event[m.get("event_ticker", m.get("ticker"))].append(m)

    best_event = max(by_event.values(), key=len)

    if len(best_event) > 1:
        rows = []
        for m in best_event:
            pct = kalshi_price_pct(m)
            title = m.get("yes_sub_title") or m.get("title", "")
            if pct is not None:
                rows.append((pct, title))
        rows.sort(reverse=True)
        lines = [f"{title}: {pct}%" for pct, title in rows[:6]]
        return "Kalshi odds:\n" + "\n".join(lines)
    ...
    if pct is None:
        return f"Found \"{title}\" on Kalshi but couldn't read a price."
    return f"{title}: {pct}% (Kalshi market-implied probability)"
...
COMMAND_LIST_TEXT = (
    "SCORES (NFL/MLB/NBA/NHL)\n"
    "[team] - score, live details + win%\n"
    "score - all live games\n"
    "[team] update / stop update\n"
    "turn on / stop - track everything\n\n"
    "STATS\n"
    "[team] standings / division\n"
    "[team] playoffs - playoff odds\n"
    "[team] roster\n"
    "predict [team] / odds / spread\n"
    "[team] injuries\n"
    "[team] countdown / next [team] game\n"
    "h2h [team1] [team2]\n"
    "streak [team]\n"
    "[team] home away\n"
    "clutch [team]\n"
    "trade deadline (nfl/mlb)\n"
    "kalshi [topic]\n\n"
    "PLAYER STATS\n"
    "pca / ohtani / caleb williams stats\n"
    "[any player] stats - any live game\n"
    "[last name] points - fantasy pts\n\n"
    "FANTASY\n"
    "fantasy / fantasy [label] / fantasy check\n"
    "fantasy roster / last week / projected\n"
    "bye week [N]\n"
    "switch [player1] and [player2]\n"
    "add [player] (drop [player2])\n"
    "qb / rb / wr / te / dst / k - top free agents\n\n"
    "AI & SEARCH\n"
    "hey bot [anything]\n"
    ". [question] or search [question]\n"
    "roast me / roast [name]\n\n"
    "UTILITIES\n"
    "weather / email / news [topic]\n"
    "remind 30m [msg]\n"
    "track [number] / flight [number]\n"
    "translate [text] / hebrew\n"
    "stock [ticker] / status / summary\n\n"
    "FUN\n"
    "joke / riddle / trivia\n"
    "would you rather / 8ball\n"
    "roll dice / flip coin\n"
    "scramble / emoji quiz\n"
    "guess the number / drink\n"
    "[x] and [y] combo - cocktail check\n"
    "N shots of X% - beer equivalent"
)
...
# Disabled per request - no more automatic morning summary message.
...
def find_two_teams(text):
    """Find two distinct teams mentioned in text, in order of appearance."""
    candidates = []
    for alias, val in sorted(TEAM_ALIASES.items(), key=lambda x: -len(x[0])):
        idx = text.find(alias)
        if idx != -1:
            candidates.append((idx, val))
    seen_abbrs = {}
    for idx, val in sorted(candidates, key=lambda c: c[0]):
        abbr = val[2]
        if abbr not in seen_abbrs:
            seen_abbrs[abbr] = (idx, val)
        if len(seen_abbrs) == 2:
            break
    if len(seen_abbrs) < 2:
        return None
    ordered = sorted(seen_abbrs.values(), key=lambda x: x[0])
...
def callback():
    ...
    if "roster" in text and "fantasy" not in text:
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(get_team_roster(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"bears roster\".")
        return "ok", 200

    if stripped.startswith("check injury ") or stripped.startswith("verify injury "):
        player_name = stripped.split(" ", 2)[2].strip()
        if player_name:
            post_to_groupme(check_injury_conflict(player_name))
        else:
            post_to_groupme("Try \"check injury caleb williams\".")
        return "ok", 200
    ...
    if "schedule" in text:
        ...
        if team:
            sport, league, abbr = team
            post_to_groupme(get_team_schedule(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"bears schedule\".")
        return "ok", 200

    if stripped.startswith("leaders "):
        rest = stripped[len("leaders "):].strip()
        team = match_team(text)  # not used for filtering, just to steer sport/league guess
        parts = rest.split()
        count = 10
        if parts and parts[-1].isdigit():
            count = int(parts[-1])
            parts = parts[:-1]
        stat_query = " ".join(parts)
        if not stat_query:
            post_to_groupme("Tell me a stat, e.g. \"leaders hr\" or \"leaders passing yards 25\".")
            return "ok", 200
        sport, league = ("baseball", "mlb")
        if "nfl" in text or any(k in text for k in ("pass", "rush", "sack", "td", "interception")):
            sport, league = ("football", "nfl")
        if "mlb" in text:
            sport, league = ("baseball", "mlb")
        post_to_groupme(get_stat_leaders(sport, league, stat_query, count))
        return "ok", 200

    if stripped.startswith("h2h"):
        two = find_two_teams(text)
        if two:
            (sport1, league1, abbr1), (sport2, league2, abbr2) = two
            post_to_groupme(get_h2h(sport1, league1, abbr1, abbr2))
        else:
            post_to_groupme("Tell me two teams, e.g. \"h2h cubs cardinals\".")
        return "ok", 200

    if stripped.startswith("streak "):
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(get_streak(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"streak cubs\".")
        return "ok", 200

    if "home away" in text or "home/away" in text:
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(get_home_away_split(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"cubs home away\".")
        return "ok", 200

    if "trade deadline" in text:
        if "nfl" in text:
            post_to_groupme(get_trade_deadline_countdown("nfl"))
        elif "mlb" in text:
            post_to_groupme(get_trade_deadline_countdown("mlb"))
        else:
            post_to_groupme(get_trade_deadline_countdown("nfl") + "\n\n" + get_trade_deadline_countdown("mlb"))
        return "ok", 200

    if "scoring plays" in text:
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(get_scoring_plays(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"cubs scoring plays\".")
        return "ok", 200

    if stripped.startswith("clinch ") or stripped.startswith("magic number"):
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(get_magic_number(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"clinch cubs\".")
        return "ok", 200

    if stripped.startswith("trade ") and " for " in stripped:
        rest = stripped[len("trade "):]
        side_a_text, side_b_text = rest.split(" for ", 1)
        side_a_names = [n.strip() for n in side_a_text.split(",") if n.strip()]
        side_b_names = [n.strip() for n in side_b_text.split(",") if n.strip()]
        if side_a_names and side_b_names:
            post_to_groupme(analyze_trade(side_a_names, side_b_names))
        else:
            post_to_groupme("Try \"trade caleb williams for josh allen\".")
        return "ok", 200

    if stripped.startswith("clutch "):
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(get_clutch_record(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"clutch cubs\".")
        return "ok", 200
    ...
    if stripped.startswith("playoffs debug"):
        team = match_team(text)
        if team:
            sport, league, abbr = team
            post_to_groupme(debug_playoff_odds(sport, league, abbr))
        else:
            post_to_groupme("Tell me which team, e.g. \"cubs playoffs debug\".")
        return "ok", 200
    ...
    if re.search(r"\d+\s*shots?\s*(?:of\s*)?\d+\s*%", stripped) or ("shot" in stripped and "%" in stripped):
        parsed = parse_shots_query(stripped)
        if parsed:
            shot_count, abv = parsed
            post_to_groupme(alcohol_conversion(shot_count, abv))
        else:
            post_to_groupme("Try something like \"4 shots of 40%\".")
        return "ok", 200

    if " and " in stripped and any(w in stripped for w in ("combo", "mix well", "good together")):
        ing1, remainder = stripped.split(" and ", 1)
        for stopword in ("good combo", "combo", "good together", "mix well", "mix"):
            remainder = remainder.replace(stopword, "")
        ing2 = remainder.strip()
        if ing1 and ing2:
            post_to_groupme(check_alcohol_combo(ing1.strip(), ing2))
        else:
            post_to_groupme("Try something like \"vodka and orange juice combo\".")
        return "ok", 200
    ...
    if stripped.startswith("kalshi "):
        topic = stripped[len("kalshi "):].strip()
        post_to_groupme(kalshi_search(topic))
        return "ok", 200
    ...
    if "news" in text:
        topic = re.sub(r"\bnews\b", "", text).strip()
    ...
    if stripped.endswith(" stats") and match_team(text) is None:
        name = stripped[: -len(" stats")].strip()
        if name:
            post_to_groupme(search_player_stats_anywhere(name))
            return "ok", 200
    ...
    if stripped in POSITION_SLOT_IDS:
        teams = parse_espn_teams()
        if not teams:
            post_to_groupme("Fantasy team isn't set up yet.")
        else:
            league_id, _, _ = teams[0]
            post_to_groupme(get_top_free_agents(league_id, stripped))
        return "ok", 200

    if stripped.startswith("add ") and "fantasy" not in stripped:
        rest = stripped[len("add "):].strip()
        teams = parse_espn_teams()
        if not teams:
            post_to_groupme("Fantasy team isn't set up yet.")
        else:
            league_id, team_id, _ = teams[0]
            if " drop " in rest:
                add_name, drop_name = rest.split(" drop ", 1)
                post_to_groupme(add_free_agent(league_id, team_id, add_name.strip(), drop_name.strip()))
            else:
                post_to_groupme(add_free_agent(league_id, team_id, rest))
        return "ok", 200

    if stripped.startswith("switch ") and " and " in stripped:
        rest = stripped[len("switch "):]
        name1, name2 = rest.split(" and ", 1)
        teams = parse_espn_teams()
        if not teams:
            post_to_groupme("Fantasy team isn't set up yet.")
        else:
            league_id, team_id, _ = teams[0]
            post_to_groupme(switch_lineup_players(league_id, team_id, name1.strip(), name2.strip()))
        return "ok", 200

    if "bye week" in text:
        match = re.search(r"bye week\s*(\d+)", text)
        if match:
            teams = parse_espn_teams()
            if not teams:
                post_to_groupme("Fantasy team isn't set up yet.")
                ...
                league_id, team_id, _ = teams[0]
                post_to_groupme(get_bye_week_players(league_id, team_id, int(match.group(1))))
        else:
            post_to_groupme("Try \"bye week 5\".")
        return "ok", 200

    if "fantasy roster" in text or "my roster" in text:
        teams = parse_espn_teams()
        if not teams:
            post_to_groupme("Fantasy team isn't set up yet.")
        else:
            league_id, team_id, _ = teams[0]
            mode = "projected" if "projected" in text else "actual"
            week = None
            if "last week" in text:
                current_status_period = None
                try:
                    cookies, headers = espn_fantasy_auth()
                    r = requests.get(
                        f"https://fantasy.espn.com/apis/v3/games/ffl/seasons/{ESPN_SEASON}/segments/0/leagues/{league_id}",
                    ...
                    current_status_period = r.json().get("status", {}).get("currentMatchupPeriod")
                except Exception:
                    pass
                week = (current_status_period - 1) if current_status_period else None
            entries, err = get_fantasy_roster_detail(league_id, team_id, week=week, mode=mode)
            if err:
                post_to_groupme(err)
            else:
                label = "last week" if "last week" in text else ("projected" if mode == "projected" else "this week")
                post_to_groupme(format_roster_text(entries, label))
        return "ok", 200
    ...
    if "standing" in text or "division" in text:
        ...
        wanted_leagues = []
        if "nfl" in text:
            wanted_leagues.append(("NFL", get_nfl_standings))
        if "mlb" in text:
            wanted_leagues.append(("MLB", get_mlb_standings))
        if "nba" in text:
            wanted_leagues.append(("NBA", get_nba_standings))
        if "nhl" in text:
            wanted_leagues.append(("NHL", get_nhl_standings))

        if not wanted_leagues:
            wanted_leagues = [("NFL", get_nfl_standings), ("MLB", get_mlb_standings)]

        for label, fn in wanted_leagues:
            post_to_groupme(f"{label} STANDINGS\n\n{fn()}" if len(wanted_leagues) > 1 else fn())
        return "ok", 200
    ...
    if "score" in text:
        ...
        nba = get_league_scores("basketball", "nba")
        nhl = get_league_scores("hockey", "nhl")
