...
def get_fantasy_roster_detail(league_id, team_id, week=None, mode="actual"):
    ...
    if resp.status_code != 200:
        return None, f"ESPN returned status {resp.status_code} - check your ESPN_S2/ESPN_SWID cookies. Response: {resp.text[:150]}"

    try:
        data = resp.json()
    except Exception:
        return None, f"Got a non-JSON response - likely a bad or expired ESPN_S2/ESPN_SWID cookie. Response start: {resp.text[:150]}"
