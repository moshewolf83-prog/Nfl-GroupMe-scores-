...
def _collect_standing_groups(node, groups):
    """Recursively walk ESPN's standings tree and collect only leaf-level
    (actual division) groups - skips league/conference-level aggregates
    that also carry a combined standings list."""
...
def get_playoff_odds(sport, league, abbr):
    url = f"https://site.api.espn.com/apis/fitt/v3/sports/{sport}/{league}/fpi"
    try:
...
def get_hebrew_lesson(word=None):
    if word:
        prompt = (
            f"Teach me the Hebrew word or phrase for \"{word}\". Give: "
            "1) the Hebrew script, 2) how to pronounce it written phonetically in English letters, "
            "3) a short example sentence using it with translation. Keep it under 5 lines total."
        )
    else:
        prompt = (
            "Give me one bite-sized Hebrew lesson for today: a useful word or short phrase. Include: "
            "1) the Hebrew script, 2) phonetic pronunciation in English letters, "
            "3) the meaning, 4) a short example sentence with translation. Keep it under 5 lines total."
        )
    reply = get_ai_reply(
        prompt,
        system_prompt="You are a friendly, concise Hebrew tutor teaching a beginner over text message. Keep lessons short and practical.",
    )
    return reply or "Hebrew lessons need the AI key set up first - ask whoever runs the bot to add OPENROUTER_API_KEY."
...
def send_daily_heartbeat():
    while True:
        ...
        if now_ct.hour == 8:  # 8am Central
            ...
            for bid in bot_ids:
                _post_to_groupme("Still running fine. (Daily heartbeat check)", bot_id=bid)
