...
# ---- Track ALL live games (NFL + MLB) mode ----
all_games_watch = {"active": False, "known": {}}


def toggle_all_games(on):
    with watch_lock:
        all_games_watch["active"] = on
        if on:
            all_games_watch["known"] = {}
        ...
        for sport, league in (("football", "nfl"), ("baseball", "mlb")):
            try:
                ...
                for event in resp.json().get("events", []):
                    ...
                    event_id = event.get("id")
                    with watch_lock:
                        prev = all_games_watch["known"].get(event_id)
                        if prev is not None and prev != line:
                            post_to_groupme(f"UPDATE: {line}")
                        all_games_watch["known"][event_id] = line
            except Exception:
                continue
